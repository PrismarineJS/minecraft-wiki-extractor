...
{
  async.waterfall([
      function(cb){id_table_parser.parseDataValues(mcpe ? "Pocket_Edition_data_values#Block_IDs" : "Data_values/Block_IDs",date,cb)},
...
{
  if(text.toLowerCase()=="nothing" || text.toLowerCase()=="none" || text.toLowerCase()=="n/a" || text.toLowerCase()=="no")
    return [];
  if(text.indexOf("{")==-1)
    return [{"drop":nameIndex.nameToId(replaceName(text.trim()))}];
  var templates=text.split("}}{{Drop").join("}};{{Drop").split(";");
  return templates
    ...
    .map(function(data){
      ...
      var min=data.simpleParts[2].indexOf("%")!=-1 ? parseFloat(data.simpleParts[2].replace("%",""))/100 :
...
function parseBlockInfobox(page,content)
{
  ...
  {
    transparent=true;
    var t=values["transparent"].toLowerCase();
    if(values["transparent"].toLowerCase()=="yes")
      filterLight=0;
    else if(t=="partial <small>(blocks light)</small>")
      filterLight=15;
    else if(t=="partial <small>(-2 to light)</small>")
      filterLight=2;
    else if(t=="partial <small>(diffuses sky light)</small>")
      filterLight=0;
    else if(t=="partial <small>(doesn\'t block light)</small>")
      filterLight=0;
    else if(t=="partial <sub>(when active)</sub>")
      filterLight=0;
    else if(t=="opaque, but lets light pass through")
      filterLight=0;
    else {
      filterLight=0;
  ...
  {
    var n=parseInt(values["light"].split(",")[1]);
    if(!isNaN(n) && n!=null) {
      emitLight = n;
    }
  }
  ...
  return {
    ...
    wikitypeToBoundingBox ? wikitypeToBoundingBox[values["type"].trim().toLowerCase()] : "block",
    "drops":p,
...
function chooseCorrectHarvestTools(tool,tool2,harvestTools,harvestTools2,material)
{
  ...
  if(material=="wood" || material == "plant")
...
function getMaterials(date,cb)
{
  wikiTextParser.getFixedArticle("Breaking",date> new Date("2015-04-09T00:00:00Z") ? date : new Date("2015-04-09T00:00:00Z"),function(err,data){
    ...
    tableContent.forEach(function(line){
      ...
      {
        var dt=parseBlockItemTemplate(line);
...
var hardnessNull=["dandelion","doublestoneslab","acacialeaves","acaciawood","sunflower","sprucefencegate","birchfencegate","junglefencegate","darkoakfencegate",
...
function addHardness(blocks,date,cb)
{
  getHardnessValues(date,function(err,linkToHardness){
    cb(null,blocks.map(function(block){
      ...
      if(!(key1 in linkToHardness) && !(key2 in linkToHardness) && hardnessNull.indexOf(key1)==-1)
        console.log("hardness not found for : "+key1+" "+key2);
      block["hardness"]=hardnessNull.indexOf(key1)!=-1 ? 0 : (key1 in linkToHardness ? linkToHardness[key1] :
        (key2 in linkToHardness ? linkToHardness[key2] : 0));
