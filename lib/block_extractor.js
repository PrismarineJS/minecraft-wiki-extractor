...
var idTableParser = require('./common/id_table_template_parser.js')
var infoboxFieldParser = require('./common/infobox_field_parser.js')
...
function parseDrops (text) {
  ...
  return templates
    ...
    .map(function (data) {
      ...
      var min = data.simpleParts[2].indexOf('%') !== -1
        ? parseFloat(data.simpleParts[2].replace('%', '')) / 100
        : parseInt(data.simpleParts[2])
      var max = data.simpleParts.length > 3
...
function parseBlockInfobox (page, content) {
  ...
  var stackSize = infoboxFieldParser.parseStackable(values.stackable)
  if (stackSize === null) {
...
function addMaterial (blocks, date, cb) {
  getMaterials(date, function (err, data) {
    ...
    var blockToMaterial = data.blockToMaterial
    // console.log(blockToMaterial);
