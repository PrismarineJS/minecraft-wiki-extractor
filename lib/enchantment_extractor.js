  ...
  wikiTextParser.getFixedArticle('Enchanting', date, function(err, data) {
    ...
    var enchantments = contentFirstTable
      ...
      .map(function(line, index) { // we map what we split and filtered above
            ...
            // we filter the languagefile for the enchatnments display name to get the internal name
...
function parseLanguagefileItem(field) {
    const regex = /[0-9a-zA-Z_]*\.minecraft.([0-9a-zA-Z_]*)/;
    const match = field.match(regex);
    if (match == null) {
        console.log("ERROR: We could not parse the language file entry " + field);
        return false;
    }
    // the language file splits names with a . but we need a :
    const newName = "minecraft:" + match[1];
    return newName;
}
...
function parseItems(fields, languageFile) {
    // this is the regex to parse the Wiki 
    const regex = /\{\{ItemSprite\|([a-zA-Z ]*?)\|link=[a-zA-Z ]*?\}\}/g;
    let m;
    var results = [];

    // iterate the field array and apply regex
    while ((m = regex.exec(fields)) !== null) {
        ...
        m.forEach((match, groupIndex) => {
            if (groupIndex == 1) { // we take only the (...) match instead of the whole thing
                // we need to check if we have a result in the language file. Otherwise the enchantment is invalid for the given version
                // this checks in the value (entry[1]) of the array
                const itemNameMatch = languageFile.find(entry => entry[1] === match);
                ...
                if (typeof thisItemName !== 'undefined') {
                    ...
                    // lets check if there are alternative materials of the same item
                    allMaterials = findItemMAterialAlternatives(fixedItemName, languageFile)
                    if (allMaterials) { // yes, add all materials
                        results = results.concat(allMaterials);
                    } else { // no, add only what we found so far.
...
// Since the Wiki only lists diamond... variety of each item, we need to get all the alternate materials
// the language file has items listed as "item.minecraft.chainmail_helmet": "Chainmail Helmet",
// out of which e.g. a sword can be made and add them to the list so that it's complete.
function findItemMAterialAlternatives(item, languageFile) {

    // let's get the "sword" from "minecraft.diamond_sword"
    const regex = /minecraft\.diamond_([a-z]*)$/g;

    m = regex.exec(item);

    // in case this is not a diamond... at all, return
    if (m == null) { 
        return false;
    }
    itemType = m[1];

    // now we have the item type, search the language file for alternatives
    // this list might need to me expanded. We might want to save this somewhere more central?
    var materials = ['netherite', 'golden', 'iron', 'wood', 'stone', 'leather', 'chainmail'];
    // now we create all types, check if they exist in the language file and return them

    var newMaterials = [];
    for (var i = 0; i < materials.length; i++) {
        // let's create a potential new material item
        newMaterial = 'item.minecraft.' + materials[i] + '_' + itemType;
        // check if it's in the language file
        const itemNameMatch = languageFile.find(entry => entry[0] === newMaterial); // entry[0] matches the key of the array
        const thisItemName = itemNameMatch && itemNameMatch[0];
        if (typeof thisItemName !== 'undefined') {
            newMaterials.push('minecraft.' + materials[i] + '_' + itemType);
        } 
    }
