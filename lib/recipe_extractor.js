...
function imp(existingItems,existingBlocks) {
  ...
  return {
    writeAllRecipes:writeAllRecipes
  };
}
...
function writeAllRecipes(file,date,cb)
{
  async.waterfall([
     ...
     //function(recipes,cb){console.log(recipes);cb(null,recipes);},
      ...
      removeConsolePocket,
      function(recipes,cb){getAliases(date,function(err,aliases){cb(null,recipes,aliases);});},
      recipesNameToId,
      formatRecipes,
      removeDuplicates,
      indexRecipes
        ...
        newRecipe[key]=/*removeAllTheSame(*/names
          ...
          .map(function(name){
            ...
            return count!=null && (count!=1 || key=="Output") && (typeof id != "undefined") ?
              id instanceof Object ? {"count":count,"id":id["id"],"metadata":id["metadata"]} :
              {"count":count,"id":id,"metadata":0}
              : id;
          })/*)*/;
      }
      else
...
{
  wikiTextParser.getFixedArticle("Module:Inventory_slot/Aliases",date,function(err,data){
    var lines=data.split("\n");
    var coloredDyes=lines.find(function(line){return line.indexOf("coloredDyes")!=-1})
      .split("=")[1].replace(/'/g,"").replace(/\[.+\]/,"").trim();
    var dyedWool=lines.find(function(line){return line.indexOf("Dyed Wool")!=-1}).
      split("=")[1].replace(/'/g,"").replace(/\[.+\]/,"").trim();
    var anys=lines
      .filter(function(line){return line.indexOf("'Any")!=-1 && line.indexOf("coloredDyes")==-1
        && line.indexOf("dyedWool")==-1 && line.indexOf("Any Firework Star")==-1
        && line.indexOf("local")==-1 && line.indexOf("aliases")==-1})
      .reduce(function(anys,any){
        var matches=any.match(/\['(.+)'\] = '(.+)'/);
        if(!matches) console.log(any);
