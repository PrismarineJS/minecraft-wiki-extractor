    ...
    function (err, info) {
      ...
      var recipes = rawRecipes
        ...
        .map(function (arrayRecipe) {
          var recipeObject = {}
          for (var i = 0; i < keys.length; i++) { recipeObject[keys[i]] = arrayRecipe[i] }
          return recipeObject
        })
      cb(null, recipes)
    }
            ...
            .map(function (name) {
              ...
              return count !== null && (count !== 1 || key === 'Output') && (typeof id !== 'undefined')
                ? id instanceof Object
                    ? { count: count, id: id.id, metadata: id.metadata }
...
var toJSON = fs.readFileSync(path.join(__dirname, 'common/toJSON.lua'), 'utf8')
